...
def train(model, device, dataloader, optimizer, criterion, clip=1.0, teacher_forcing_ratio=0.5):
    ...
    for batch in dataloader:
        ...
        # Forward pass
        output, _ = model(source, source_length, target, teacher_forcing_ratio)
...
def evaluate(model, device, dataloader, criterion):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # Forward pass
            output, _ = model(source, source_length, target, teacher_forcing_ratio=0)
...
def transliterate(model, device, dataset, source_text, max_length=50):
    ...
    with torch.no_grad():
        ...
        predicted_indices, _ = model.inference(source_tensor, source_length, sos_idx, eos_idx, max_length)
